...
function differentiate(a::HomogeneousPolynomial, r::Int)
    ...
    T = TS.numtype(a)
    get_order(a) == 0 && return HomogeneousPolynomial(sp, zero(a[1]), 0)
    @inbounds num_coeffs = sp.size_table[get_order(a)]
    coeffs = zeros(T, num_coeffs)
    @inbounds posTb = sp.pos_table[get_order(a)]
    @inbounds num_coeffs = sp.size_table[get_order(a)+1]
    ct = zeros(Int, get_numvars(sp), num_coeffs)
    for i = 1:num_coeffs
        ct[:, i] .= sp.coeff_table[get_order(a)+1][i][:]
    end
    @inbounds for i = 1:num_coeffs
        ...
        n == 0 && continue
        iind[r] -= 1
        kdic = in_base(get_order(sp), iind)
        pos = posTb[kdic]
        coeffs[pos] = n * a[i]
        iind[r] += 1
    end

    return HomogeneousPolynomial{T}(sp, coeffs, get_order(a)-1)
end
...
function differentiate(a::TaylorN, r=1::Int)
    ...
    return TaylorN{T}(a.space, coeffs, get_order(a) )
end
...
function integrate(a::HomogeneousPolynomial, r::Int)
    sp = a.space
    @assert 1 ≤ r ≤ get_numvars(sp)
    order_max = get_order(sp)
    # NOTE: the following returns order 0, but could be get_order(), or get_order(a)
    get_order(a) == order_max && return HomogeneousPolynomial(sp, zero(a[1]/1), 0)
    @inbounds posTb = sp.pos_table[get_order(a)+2]
    @inbounds num_coeffs = sp.size_table[get_order(a)+1]
    T = promote_type(TS.numtype(a), TS.numtype(a[1]/1))
    coeffs = zeros(T, sp.size_table[get_order(a)+2])
    ct = zeros(Int, get_numvars(sp), num_coeffs)
    for i = 1:num_coeffs
        ct[:, i] .= sp.coeff_table[get_order(a)+1][i][:]
    end
    ...
    return HomogeneousPolynomial(sp, coeffs, get_order(a)+1)
end
...
function integrate(a::TaylorN, r::Int)
    ...
    order_max = min(get_order(a.space), get_order(a)+1)
