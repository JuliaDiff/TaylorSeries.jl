...
function derivative(a::TaylorN, r=1::Int)
    T = eltype(a)
    @compat coeffs = Array{HomogeneousPolynomial{T}}(undef, a.order)
...
function gradient(f::TaylorN)
    T = eltype(f)
    numVars = get_numvars()
    @compat grad = Array{TaylorN{T}}(undef, numVars)
...
function jacobian(vf::Array{TaylorN{T},1}) where {T<:Number}
    ...
    @compat jac = Array{T}(undef, numVars, numVars)
...
function jacobian(vf::Array{TaylorN{T},1}, vals::Array{S,1}) where
    ...
    @compat jac = Array{R}(undef, numVars, numVars)
