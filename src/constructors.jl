...
- `coeffs :: Memory{T}` Expansion coefficients; the ``i``-th
...
struct Taylor1{T<:Number} <: AbstractSeries{T}
        coeffs :: Memory{T}
    ## Inner constructors ##
    function Taylor1{T}(coeffs::Memory{T}) where {T<:Number}
        return new{T}(coeffs)
    end
    function Taylor1{T}(coeffs::AbstractVector{T}, order::Int) where {T<:Number}
        v = Memory{T}(undef, order+1)
...
Taylor1(coeffs::Memory{T}) where {T<:Number} = Taylor1{T}(coeffs)
Taylor1(coeffs::AbstractVector{T}, order::Int) where {T<:Number} =
    Taylor1{T}(coeffs, order)
Taylor1(coeffs::AbstractVector{T}) where {T<:Number} =
    Taylor1(Memory{T}(coeffs))
Taylor1(coeffs::Memory{T}, order::Int) where {T<:Number} =
    Taylor1{T}(coeffs, order)
...
function Taylor1(::Type{T}, order::Int) where {T<:Number}
    coeffs = Memory{T}(undef, order+1)
    coeffs .= zero(T)
    coeffs[2] = one(T)
    return Taylor1{T}(coeffs)
end
function Taylor1(order::Int)
    coeffs = Memory{Float64}(undef, order+1)
    coeffs .= 0.0
    coeffs[2] = 1.0
...
- `coeffs  :: Memory{T}` Expansion coefficients of the homogeneous
...
struct HomogeneousPolynomial{T<:Number} <: AbstractSeries{T}
    ...
    HomogeneousPolynomial{T}(coeffs::DenseVector{T}, order::Int) where
        {T<:Number} = new{T}(_coeffsHP(coeffs, order), order)
end

HomogeneousPolynomial(x::HomogeneousPolynomial{T}) where {T<:Number} = x
HomogeneousPolynomial(coeffs::DenseVector{T}, order::Int) where {T<:Number} =
    HomogeneousPolynomial{T}(coeffs, order)
HomogeneousPolynomial(coeffs::DenseVector{T}) where {T<:Number} =
...
- `coeffs  :: Memory{HomogeneousPolynomial{T},1}` Vector containing the
...
struct TaylorN{T<:Number} <: AbstractSeries{T}
        coeffs  :: Memory{HomogeneousPolynomial{T}}
    ## Inner constructor ##
    function TaylorN{T}(v::DenseVector{HomogeneousPolynomial{T}},
...
function TaylorN(x::DenseVector{HomogeneousPolynomial{T}},
        order::Int) where {T<:Number}
    if order == 0
        order = maxorderH(x)
    end
...
TaylorN(x::DenseVector{HomogeneousPolynomial{T}}) where {T<:Number} =
...
Taylor1{T}(x::S) where {T<:Number, S<:NumberNotSeries} =
    Taylor1([convert(T,x)], 0)
TaylorN{T}(x::S) where {T<:Number, S<:NumberNotSeries} =
    TaylorN(convert(T, x), get_order())
...
get_numvars(::T) where {T<:Union{HomogeneousPolynomial, TaylorN}} =
    _params_TaylorN_.num_vars
