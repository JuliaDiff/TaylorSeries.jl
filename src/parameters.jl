...
```
"""
function set_variables(R::Type, names::Vector{T}; order=get_order()) where T<:AbstractString
...
function set_variables(R::Type, names::T; order=get_order(), numvars=-1) where T<:AbstractString
