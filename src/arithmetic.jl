...
function zeros(::HomogeneousPolynomial{T}, order::Int) where {T<:Number}
    ...
    @compat v = Array{HomogeneousPolynomial{T}}(undef, order+1)
...
for (f, fc) in ((:+, :(add!)), (:-, :(subst!)))
    ...
    @eval begin
        ...
        function ($f)(a::TaylorN{Taylor1{T}}, b::Taylor1{S}) where
            ...
            R = eltype(aux)
            @compat coeffs = Array{HomogeneousPolynomial{Taylor1{R}}}(undef, a.order+1)
            coeffs .= a.coeffs
        ...
        function ($f)(a::Taylor1{TaylorN{T}}, b::TaylorN{S}) where
            ...
            R = eltype(aux)
            @compat coeffs = Array{TaylorN{R}}(undef, a.order+1)
            coeffs .= a.coeffs
            @inbounds coeffs[1] = aux
    ...
    @eval begin
        ...
        *(a::$T, b::Bool) = b * a
        ...
        function *(a::T, b::$T) where {T<:NumberNotSeries}
            ...
            @compat v = Array{typeof(aux)}(undef, length(b.coeffs))
...
for T in (:HomogeneousPolynomial, :TaylorN)
    ...
    @eval begin
        function *(a::Taylor1{T}, b::$T{Taylor1{S}}) where
            ...
            @inbounds aux = a * b.coeffs[1]
            R = typeof(aux)
            @compat coeffs = Array{R}(undef, length(b.coeffs))
...
function /(a::Taylor1{Rational{T}}, b::S) where {T<:Integer, S<:NumberNotSeries}
    R = typeof( a[0] // b)
    @compat v = Array{R}(undef, a.order+1)
...
function mul!(y::Vector{Taylor1{T}},
    ...
    # B = zeros(T, k, order+1)
    @compat B = Array{T}(undef, k, order+1)
    B = zero.(B)
    for i = 1:k
    ...
    @compat Y = Array{T}(undef, n, order+1)
    @compat mul!(Y, a, B)
    @inbounds for i = 1:n
